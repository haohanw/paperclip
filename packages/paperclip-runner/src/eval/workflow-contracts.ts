...
export type RunnerWorkflowLane = "deterministic" | "live" | "chaos";
...
export interface RunnerWorkflowEvalCase {
  ...
  title: string;
  version: 1;
  lanes: RunnerWorkflowLane[];
  tags: string[];
  coreLiveRepeat: boolean;
...
export function assertRunnerWorkflowEvalCase(value: RunnerWorkflowEvalCase): void {
  ...
  if (!unique(value.tags) || !unique(value.providers) || !unique(value.lanes)) {
